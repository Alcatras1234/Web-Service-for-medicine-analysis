...
public class ReportService {
    ...
    private String generateAndUploadHeatmap(Job job, List<PatchTask> patches) throws Exception {
        ...
        for (PatchTask patch : patches) {
            ...
            if (count > 0 && pw > 15 && ph > 15) {
                ...
                int fontSize = Math.max(7, Math.min(14, pw / 3));
        ...
        if (job.getMaxHpfX() != null && job.getMaxHpfY() != null) {
            ...
            int pw = (int) (448 * scale);
            int ph = (int) (448 * scale);
    ...
    private void drawLegend(Graphics2D g, int imgW, int imgH, int maxCount) {
        int legX = imgW - 120, legY = imgH - 160;
        int legW = 20, legH = 120;

        for (int i = 0; i < legH; i++) {
            float ratio = 1f - (float) i / legH;
    ...
    private String generateAndUploadPdf(Job job, List<PatchTask> patches,
        ...
        DeviceRgb diagColor = isPositive
                ? new DeviceRgb(180, 0, 0)
                : new DeviceRgb(0, 120, 0);
        ...
        addTableRow(table, "Координаты макс. HPF (x, y)",
                job.getMaxHpfX() + ", " + job.getMaxHpfY());
        ...
        long failedCount = patches.stream()
                .filter(p -> "FAILED".equals(p.getStatus())).count();
        addTableRow(table, "Патчей с ошибкой", String.valueOf(failedCount));
        ...
        if (heatmapMinioPath != null) {
            ...
            doc.add(new Paragraph(
                    ...
                    "Красная рамка — зона с наибольшей плотностью.")
                    .setFontSize(9).setFontColor(ColorConstants.GRAY));
            try {
                byte[] heatmapBytes = downloadBytes(heatmapMinioPath);
                Image heatmapImg = new Image(ImageDataFactory.create(heatmapBytes));
                heatmapImg.setWidth(UnitValue.createPercentValue(100));
                doc.add(heatmapImg);
            } catch (Exception e) {
                log.warn("Could not attach heatmap to PDF: {}", e.getMessage());
                doc.add(new Paragraph("[Heatmap недоступен]")
                        .setFontColor(ColorConstants.GRAY));
            }
        }
        ...
        List<PatchTask> top5 = patches.stream()
                .filter(p -> p.getEosinophilCount() != null && p.getEosinophilCount() > 0)
                .sorted((a, b) -> Integer.compare(b.getEosinophilCount(), a.getEosinophilCount()))
                .limit(5)
                .toList();

                for (int i = 0; i < top5.size(); i++) {
            PatchTask p = top5.get(i);
            boolean isMax = i == 0;
            DeviceRgb rowColor = isMax
                    ? new DeviceRgb(255, 235, 235)
                    : new DeviceRgb(255, 255, 255);

            topTable.addCell(new Cell().add(new Paragraph(String.valueOf(i + 1)))
            ...
            topTable.addCell(new Cell().add(new Paragraph(String.valueOf(p.getY())))
                    .setBackgroundColor(rowColor));
            topTable.addCell(new Cell().add(
                    new Paragraph(String.valueOf(p.getEosinophilCount())).setBold())
                    .setBackgroundColor(rowColor));
        ...
        if (job.getMaxHpfX() != null) {
            doc.add(new Paragraph("Изображение зоны с максимальной плотностью (HPF max)")
                    .setFontSize(12).setBold());
            doc.add(new Paragraph(
                    "Координаты: x=" + job.getMaxHpfX() + ", y=" + job.getMaxHpfY() +
                    "  |  Эозинофилов: " + job.getMaxHpfCount())
                    .setFontSize(10).setFontColor(ColorConstants.GRAY));

            patches.stream()
                    .filter(p -> p.getX() == job.getMaxHpfX() && p.getY() == job.getMaxHpfY())
                    .findFirst()
                    .ifPresent(maxPatch -> {
                        try {
                            byte[] patchBytes = downloadBytes(maxPatch.getMinioPath());
                            Image patchImg = new Image(ImageDataFactory.create(patchBytes));
                            patchImg.setWidth(UnitValue.createPercentValue(50));
                            doc.add(patchImg);
                        } catch (Exception e) {
                            log.warn("Could not attach max patch image: {}", e.getMessage());
                        }
                    });
        }
